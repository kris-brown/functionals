...
def chain_rule(df_dgx: Unary, dg_dx: Unary, g: Unary) -> Unary:
    '''Return function which evaluates d(f(g(x)))/dx at a point x'''
    def f(x: float) -> float:
        return dg_dx(x) * df_dgx(g(x))
    return f
...
def chain_rule2(df2_dgx2: Unary, df_dgx: Unary, dg2_dx2: Unary, dg_dx: Unary,
    ...
    '''
    def f(x: float) -> float:
        return dg_dx(x)**2 * df2_dgx2(g(x)) + dg2_dx2(x)*df_dgx(g(x))
    return f
...
# def g_s(a1: float = 4.9479) -> Unary:
#     def f(x: float) -> float:
#         return 1. if x == 0 else 1. - float(np.exp(-a1*np.power(x, -0.5)))
#     return f
...
def LegProduct(s: float, a: float, A: np.ndarray = None) -> np.ndarray:
    ...
    """
    # gs = g_s(a1)(s)
    out = [[ps*pa for pa in LegVals(t_s(s), 8)]
           for ps in LegVals(t_alpha(a), 8)]
...
# def dg_ds(a1: float = 4.9479) -> Unary:
#     '''This blows up at x=0'''
#     def f(x: float) -> float:
#         if x == 0:
#             return 0
#         num = a1 * float(np.exp(-a1*np.power(x, -0.5)))
#         return -0.5*num/x**1.5
#     return f


def dt_da(alpha: float) -> float:
    n = 3*alpha*(2*alpha**4+alpha**3+alpha+2)*(1-alpha**2)**2
    d = (alpha**6 + alpha**3 + 1)**2
    return n/d
...
def t_alpha_inv(a_: float) -> float:
    assert a_ <= 1 and a_ >= -1
    res = opt.root_scalar(f=lambda x: t_alpha(x)-a_, x0=10**a_ - 0.098,
                          x1=np.exp(0.5*a_)-0.1)
...
if __name__ == '__main__':
    ...
    dl4s = [chain_rule(dL_dx(5, lv), dt_ds, t_s)(s)
            for lv, s in zip(lvs, ss)]
    import plotly
    import plotly.graph_objs as go
    data = [go.Scatter(x=ss, y=l4s, name='L5(x)'),
            go.Scatter(x=ss, y=dl4s, name='dL5(x)/dx')]
