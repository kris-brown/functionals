...
from numpy import zeros,array , exp                       # type: ignore
...
Consider being more efficient when calling Legendre Polynomials
(to evaluate order 8, you need to know orders 1-7).
...
def LegendreProduct(s      : float,
    ...
    Eq # 4 in mBEEF paper
    """
    s,alpha = [max(x,1e-30) for x in [s,alpha]]
    gx  = 1 - exp(-a1*s**(-0.5))
