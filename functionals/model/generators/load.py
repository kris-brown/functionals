...
# Internal Modules
from dbgen2 import (Model, Gen, Query, Ref, PyBlock, AND, Env, defaultEnv, Import, EQ)
...
def load(mod : Model) -> None:
    ...
    sdq   = Query(exprs={'sd'  : Job['stordir']})
    logq  = Query(exprs={'log' : Job['log']})
    rawq  = Query(exprs={'raw' : Struct['raw']})
    spcq  = Query(exprs={'c':Species['composition']})
    ########################################################################
    fs_env = defaultEnv + Env(Import('re','findall'))

    jspb = PyBlock(find_setups, env = fs_env,
                    args = [logq['log']], outnames=['check'])
    ...
    jobsetup =                                                                  \
        Gen(name    = 'jobsetup',
            ...
            funcs   = [jspb])
    ########################################################################
    psq = Query(exprs = {'pt' : Pure_struct['prototype']})
    pspb = PyBlock((lambda x: {'AB_1_a_b_225'       : 'rocksalt',
                    ...
                    args=[psq['pt']],outnames=['nn'])
    psnick =                                                                    \
        Gen(name    = 'psnick',
            desc    = 'Use a dict to map nicknames to some Pure structs',
            actions = [Pure_struct(pure_struct = Ref('pure_struct'),
                                   nickname    = pspb['nn'])],
            query   = psq,
            funcs   = [pspb],
            tags    = ['species'])
    ...
    spb1 = PyBlock(anytraj, env=anytraj_env, args=[sdq['sd']], outnames=['traj'])
    spb2 = PyBlock(traj_to_json, args=[spb1['traj']], outnames=['raw'])
    ...
    jepb = PyBlock(eng,env=eng_env,args=[logq['log']],outnames=['eng'])
    jobeng = Gen(name    = 'jobeng',
                 desc    = '?',
                 actions = [Job(job=Ref('job'),energy=jepb['eng'])],
                 query   = logq,
                 funcs   = [jepb])
    ...
    mdpb = PyBlock(metadata, env=md_env, args=[sdq['sd']], outnames=['u','t'])
    jobmetadata =                                                               \
        Gen(name    = 'jobmetadata',
            desc    = 'Scrape metadata about job',
            actions = [Job(job=Ref('job'),user=mdpb['u'],timestamp=mdpb['t'])],
            query   = sdq,
            funcs   = [mdpb])

    ########################################################################

    atomdetails = ['num','x','y','z','const','mag','tag']
    apb = PyBlock(get_atoms, args= [rawq['raw']],
                  outnames = atomdetails + ['ind'])
    atom =                                                                      \
        Gen(name    = 'atom',
            desc    = 'Initializes atoms from knowing n_atoms',
            actions = [Atom(insert = True,
                            element= Element(atomic_number = apb['num']),
                            struct = Ref('struct'),magmom=apb['mag'], ind = apb['ind'],
    ...
    elems =                                                                     \
        Gen(name    = 'elems',
            ...
            actions = [Element(insert=True,atomic_number=epb['z'])],
            funcs   = [epb])
    ...
    cell =                                                                      \
        Gen(name    = 'cell',
            ...
            actions = [Struct(struct = Ref('struct'),
    ...
    ########################################################################
    cellq   = Query(exprs = {x : Cell[x] for x in Cell.ids()})
    ...
    celinfo =                                                               \
        Gen(name    = 'celinfo',
            desc    = 'Basic geometric formulas applied to 3x3 cell '
                       'representation',
            actions = [Cell(cell=Ref('cell'),**{x:cellpb[x] for x in cpbout})],
            query   = cellq,
            funcs   = [cellpb])
    ...
    systype =                                                                   \
        Gen(name    = 'systype',
            ...
            actions = [Struct(struct=Ref('struct'),system_type=gst['st'])],
            query   =  rawq,
            funcs   =  [jtt,gst])
    ...
    psq = Query(exprs  = {'raw' : Struct['raw']},
    ...
    ps   =                                                                      \
        Gen(name    = 'ps',
            ...
            actions = [Struct(struct      = Ref('struct'),
                              pure_struct = Pure_struct(insert=True,prototype=psb3['ptype']))],
            query   = psq,
    ...
    spnq  = Query(exprs={'c':Species['composition'],'s':Species['symmetry']})
    spnpb =  PyBlock(species_nick, env = le_env + cs_env,
                     args = [spnq['c'],spnq['s']], outnames = ['nn'])
    spnick =                                                                    \
        Gen(name    = 'spnick',
            desc    = "Determine species' nicknames by analyzing their "\
                       "composition and symmetry" ,
            actions = [Species(species=Ref('species'),nickname=spnpb['nn'])],
            query   = spnq,
            funcs   = [spnpb])
    ########################################################################
    ...
    sp_comp =                                                                  \
        Gen(name    = 'sp_comp',
            ...
            funcs   = [spf])
    ########################################################################
    pops_q = Query(exprs={'c':Struct['composition_norm'],
    ...
    species_natoms =                                                            \
        Gen(name    = 'species_natoms',
            ...
            actions = [Species(species=Ref('species'),n_atoms=snapb['n'])],
            query   = spcq,
            funcs   = [snapb])
    ########################################################################
    refq = Query(exprs = {'xc':Calc['xc'],'z':Atom['number'],'e':Job['energy']},
                 basis = ['job'],
                 links = [Atom.r('struct')],
    ...
    compcols = ['n_atoms','n_elems','composition',
                     ...
                     'str_constraints']
    ca_env = Env(Import('dbgen.core.lists','normalize_list','nub'))+defaultEnv
    capb = PyBlock(countatm, env = ca_env, args=[rawq['raw']],
                    outnames = compcols)
    countatoms =                                                            \
        Gen(name    = 'countatoms',
